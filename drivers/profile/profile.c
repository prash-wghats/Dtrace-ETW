...
profile_provide(void *arg, dtrace_probedesc_t *desc)
{
	...
	const struct {
		...
		{ PROF_PREFIX_SAMPLE, PROF_SAMPLE },
		{ 0, 0 }
	};
	...
	if (desc == NULL) {
		...
		for (i = 0; i < sizeof (sample_rates) / sizeof (int); i++) {
			if ((rate = sample_rates[i]) == 0)
				continue;
			(void) snprintf(n, PROF_NAMELEN, "%s%d",
...
profile_detach()
{
	...
	if (profile_id && (error = dtrace_unregister(profile_id)) != 0)
		return (error);
#if defined(sun)
