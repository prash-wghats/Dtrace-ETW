 ...
 *
 * $FreeBSD: release/10.0.0/sys/cddl/contrib/opensolaris/uts/common/dtrace/fasttrap.c 254198 2013-08-11 00:57:01Z rpaulo $
 */
...
#if defined(windows)
#define	FASTTRAP_FPID_NAME		"fpid"
#endif
...
#if !defined(sun)
static kmutex_t *fasttrap_cpuc_pid_lock;
#endif
...
fasttrap_sigtrap(proc_t *p, kthread_t *t, uintptr_t pc)
{
	...
	if (t != NULL)
		aston(t);
#else
	dprintf("fasttrap.sys: fasttrap_sigtrap %p\n", pc);
	fasttrap_winsig(p->pid, pc);
#endif
}
...
fasttrap_pid_cleanup_cb(void *data)
{
	...
	if (later > 0) {
		if (fasttrap_timeout != (timeout_id_t)1) {
			fasttrap_timeout =
			    timeout(&fasttrap_pid_cleanup_cb, NULL, NANOSEC);
		}
...
void dtrace_helpers_destroy(void);
/*
...
fasttrap_exec_exit(proc_t *p)
{
	 ...
	 */
	fasttrap_provider_retire(p->pid, FASTTRAP_PID_NAME, 0);
...
fasttrap_tracepoint_enable(proc_t *p, fasttrap_probe_t *probe, uint_t index)
{
	...
	uintptr_t pc;
#if defined(windows)
	uint_t prov_type = strstr(probe->ftp_prov->ftp_name, "fpid") ? 1: 0;
#endif
...
#if defined(windows)
		if (prov_type) {
			if (fasttrap_tracepoint_remove_fpid(p, probe, tp) != 0)
...
#else
fasttrap_pid_enable(void *arg, dtrace_id_t id, void *parg, int stackon)
#endif
{
...
fasttrap_pid_disable(void *arg, dtrace_id_t id, void *parg)
{
	 ...
	 */
	if ((p = fasttrap_pfind(probe->ftp_pid)) != NULL) {
#ifdef __FreeBSD__
		_PHOLD(p);
		PROC_UNLOCK(p);
#endif
...
/*ARGSUSED*/
int
fasttrap_ioctl(void *arg,  int cmd)
{
#ifdef notyet
...
fasttrap_load(void)
{
	...
	mutex_init(&fasttrap_count_mtx, "fasttrap count mtx", MUTEX_DEFAULT,
	    NULL);
#endif
	mutex_init(&fasttrap_cleanup_mtx);
