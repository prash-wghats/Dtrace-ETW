...
static uintptr_t fasttrap_getreg(struct reg *, uint_t);
...
#ifdef __amd64
	if (curproc->model == DATAMODEL_LP64) {
		uintptr_t *stack;
    ...
    fasttrap_probe_type_t type)
{
	 ...
	 */
	if ((size = dtrace_instr_size_isa(instr, model, &rmindex)) <= 0)
		return (-1);
	 ...
	 */
	if (model == DATAMODEL_LP64 && (instr[start] & 0xf0) == 0x40)
		rex = instr[start++];
#endif
	...
	if (instr[start] == FASTTRAP_2_BYTE_OP) {
		...
		if (reg == 2 || reg == 4) {
			...
			ASSERT(model == DATAMODEL_LP64 || rex == 0);
...
#ifdef __amd64
	if (model == DATAMODEL_LP64 && tp->ftt_type == FASTTRAP_T_COMMON) {
		/*
	    ...
	    uintptr_t *argv)
{
	...
	for (i = 0; i < cap; i++) {
		...
		if (x < 4) {
			switch (x) {
			case 0: argv[i] = rp->r_rcx; break;
			case 1: argv[i] = rp->r_rdx; break;
			case 2: argv[i] = rp->r_r8; break;
			case 3: argv[i] = rp->r_r9; break;
			}
		} else {
			argv[i] = fasttrap_fulword_noerr(&stack[5+(x-4)]);
		}
#else
		if (x < 6)
			argv[i] = (&rp->r_rdi)[x];
...
{
	proc_t *p = curproc;
	uintptr_t pc = rp->r_rip - 1, new_pc = 0;
	fasttrap_bucket_t *bucket;
#if defined(sun)
	kmutex_t *pid_mtx;
#endif
			...
			for (id = tp->ftt_ids; id != NULL; id = id->fti_next) {
...
#if defined(windows)
				uint64_t *stack = (uint64_t *)rp->r_rsp;
				uintptr_t s5 = 0;
				s5 = fasttrap_fulword_noerr(&stack[5]);
#endif
				if (id->fti_ptype == DTFTP_ENTRY) {
					...
					dtrace_probe(probe->ftp_id, rp->r_rdi,
					    ...
					    rp->r_r8);
#else
					dtrace_probe(probe->ftp_id, rp->r_rcx,
					    rp->r_rdx, rp->r_r8, rp->r_r9,
					    s5);
#endif
...
#endif
			addr = rp->r_rsp - sizeof (uintptr_t);
			ret = fasttrap_sulword((void *)addr, rp->r_rbp);
#ifdef __amd64
		} else {
			addr = rp->r_rsp - sizeof (uint32_t);
			ret = fasttrap_suword32((void *)addr, rp->r_rbp);
		}
#endif
	...
	case FASTTRAP_T_COMMON: {
		...
		if (td->td_dtrace_sscr == NULL) {
			td->td_dtrace_sscr = scr_alloc_mem(p);
			if (td->td_dtrace_sscr == NULL) {
				fasttrap_sigsegv(p, td, pc);
				new_pc = pc;
				break;
			}
		}
		addr = (uintptr_t) td->td_dtrace_sscr;
...
fasttrap_return_probe(struct reg *rp)
{
	...
	fasttrap_return_common(rp, pc, p->pid, npc);
