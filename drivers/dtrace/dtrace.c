...
hrtime_t	dtrace_deadman_user = (hrtime_t)120 * NANOSEC;
...
static dtrace_provider_t *dtrace_provider = NULL;	/* provider list */
...
#else
static void
dtrace_nullop(void *a, void *b)
{}

static int
dtrace_enable_nullop(void *a, dtrace_id_t b, void *c)
{
	return (0);
}
static void
dtrace_nullop1(void *a, dtrace_id_t c, void *b)
{}

static dtrace_pops_t	dtrace_provider_ops = {
	(void (*)(void *, const dtrace_probedesc_t *))dtrace_nullop,
	(void (*)(void *, void *))dtrace_nullop,
	(int (*)(void *, dtrace_id_t, void *))dtrace_enable_nullop,
	(void (*)(void *, dtrace_id_t, void *))dtrace_nullop1,
	(void (*)(void *, dtrace_id_t, void *))dtrace_nullop1,
	(void (*)(void *, dtrace_id_t, void *))dtrace_nullop1,
	NULL,
	NULL,
...
#define	DTRACE_TLS_THRKEY(where) { \
	uint_t intr = 0; \
	uint_t actv = GetCurrentIrql() >> (LOCK_LEVEL + 1); \
	for (; actv; actv >>= 1) \
		intr++; \
	ASSERT(intr < (1 << 3)); \
    ...
    dtrace_mstate_t *mstate, dtrace_vstate_t *vstate)
{
	...
	for (i = 0; i < nkeys; i++) {
		if (key[i].dttk_size == 0) {
			...
			for (j = 0; j < size; j++) {
				hashval += dtrace_load8((uintptr_t)(base + j));
				hashval += (hashval << 10);
				hashval ^= (hashval >> 6);
			}
		}
...
#else
		return ((uint64_t)curthread->ppid);
#endif
	case DIF_VAR_TID:
#if defined(sun)
		/*
    ...
    dtrace_mstate_t *mstate, dtrace_state_t *state)
{
	...
	case DIF_SUBR_WSTRINGOF: {
		char *dest = (wchar_t *) mstate->dtms_scratch_ptr;
		int64_t wsz, size = state->dts_options[DTRACEOPT_STRSIZE];
		wchar_t *src = (wchar_t *) tupregs[0].dttk_value;


		if (src == NULL) {
			dest[0] = '\0';
			mstate->dtms_scratch_ptr = 1;
			regs[rd] = dest;
			break;
		}
		if (!DTRACE_INSCRATCH(mstate, size)) {
		...
		DTRACE_CPUFLAG_SET(CPU_DTRACE_NOFAULT);
		wsz = dtrace_wcstombs(dest, src, size-1);
		DTRACE_CPUFLAG_CLEAR(CPU_DTRACE_NOFAULT);

		dest[wsz++] = '\0';
		mstate->dtms_scratch_ptr += wsz;
		regs[rd] = dest;
...
#else
	if (old->dtpv_pops.dtps_enable ==
	    (int (*)(void *, dtrace_id_t, void *, int))dtrace_enable_nullop) {
#endif
		/*
...
#else
dtrace_state_create(HANDLE dev, void *p)
#endif
{
#if defined(sun)
	minor_t minor;
...
#else
	state->dts_cleaner.func = dtrace_state_clean;
	state->dts_deadman.func = dtrace_state_deadman;
	state->dts_cleaner.state = (void *) state;
	state->dts_deadman.state = (void *) state;
	callout_init(&state->dts_cleaner, dev);
	callout_init(&state->dts_deadman, dev);
#endif
...
#else
	state->dts_alive = state->dts_laststatus = dtrace_gethrtime();
	callout_reset(&state->dts_cleaner, opt[DTRACEOPT_CLEANRATE]);
	callout_reset(&state->dts_deadman, dtrace_deadman_interval);
#endif
	state->dts_activity = DTRACE_ACTIVITY_WARMUP;
...
#else
		StringCchPrintfA(c, sizeof (c), "dof-data-%d", i);
#endif
    ...
    dof_helper_t *dofhp)
{
	...
	if (!dtrace_attached() || dtrace_meta_pid == NULL) {
		...
		dtrace_helper_provide(dofhp, p->pid);
		...
		if (dtrace_meta_pid != NULL) {
			...
			for (i = 0; i < help->dthps_nprovs; i++) {
				dtrace_helper_provider_remove(
				    &help->dthps_provs[i]->dthp_prov, p->pid);
			}
		} else {
			mutex_enter(&dtrace_lock);
...
#else
	state = dtrace_state_create(dev, state);
#endif
	mutex_exit(&cpu_lock);
...
static void
dtrace_close(void *data)
{
	dtrace_state_t *state = (dtrace_state_t *) data;
...
dtrace_load(void *dummy)
{
	 ...
	 */
	mutex_init(&dtrace_lock);
	mutex_init(&dtrace_provider_lock);
	mutex_init(&dtrace_meta_lock);
	mutex_init(&cpu_lock);
#ifdef DEBUG
	mutex_init(&dtrace_errlock);
#endif
...
dtrace_unload(HANDLE DrvObj)
{
	...
	if ((state = dtrace_anon_grab()) != NULL) {
		/*
		 * If there were ECBs on this state, they will be lost.
		 * In windows there seems no way to conditonally fail in
		 * unload routine.
		 */
		dtrace_state_destroy(state);
	}
...
dtrace_ioctl_helper(int cmd, intptr_t arg, int *rv)
{
	...
	case DTRACEHIOC_ADD: {
		...
		if ((p = pfind(dhp->dofhp_pid)) == NULL)
			return (EINVAL);
		dof = dtrace_dof_copyin_proc(p, arg, &rval);
#endif
		if (dof == NULL)
			return (rval);
...
/* ARGSUSED */
int
dtrace_ioctl(void *addr,  int cmd, void *ext)
{
	dtrace_state_t *state;
	int rval, oaddr;
	...
	case DTRACEIOC_DOFGET: {
		...
		if (copyin((void *)addr, &hdr, sizeof (hdr)) != 0)
			return (EFAULT);
		...
		rval = copyout(dof, (void *) addr, len);
		dtrace_dof_destroy(dof);
	...
	case DTRACEIOC_ENABLE: {
		...
		if (dtrace_dof_slurp(dof, vstate, (cred_t *) NULL, &enab, 0, B_TRUE) != 0) {
			mutex_exit(&dtrace_lock);
			mutex_exit(&cpu_lock);
			dtrace_dof_destroy(dof);
			return (EINVAL);
	...
	case DTRACEIOC_PROBES: {
		...
		dtrace_cred2priv(NULL, &priv, &uid, &zoneid);
	...
	case DTRACEIOC_USERMOD: {
		...
		if (copyin((void *)addr, &mod, sizeof (mod)) != 0)
			return (EFAULT);

		winos_user_module_mdl(mod.pid, &mod);
		return (0);
	}
#endif
	default:
...
#if defined(windows)
int (*pdtrace_open)(HANDLE, void*) = dtrace_open;
void (*pdtrace_close)(void*) = dtrace_close;
void (*pdtrace_load)(void*) = dtrace_load;
int (*pdtrace_unload)(HANDLE) = dtrace_unload;
int (*pdtrace_ioctl)(void*, int, void*) = dtrace_ioctl;
#endif