...
sdt_provide(void *arg, dtrace_probedesc_t *desc)
{
	...
	do {
		...
		for (etw = etwp; etw != NULL && etw->etw_nprov != NULL; etw++) {
			...
			sdp->sdp_patchpoint = (sdt_instr_t *) etw->etw_guid->Data1;
		}
...
sdt_enable(void *arg, dtrace_id_t id, void *parg, int stackon)
{
	...
	if (etw->etw_bcb)
		dtrace_etw_hook_event(etw->etw_bguid, etw->etw_bcb, NULL, ETW_EVENTCB_ORDER_FRIST);
	if (etw->etw_cb)
		dtrace_etw_hook_event(etw->etw_guid, etw->etw_cb, NULL, ETW_EVENTCB_ORDER_ANY);
	if (etw->etw_eventno > 0) {

		if (sdp->sdp_provider->sdtp_etw_flags) {
			dtrace_etw_prov_enable(sdp->sdp_provider->sdtp_etw_flags);
		}
...
sdt_etw_guid_cb(PEVENT_RECORD ev, void *data)
{
	...
	sdt_probe_t *sdt = sdt_probetab[SDT_ADDR2NDX(ev->EventHeader.ProviderId.Data1)];
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_ctl->etw_kw == 0 ||
			    (sdt->sdp_ctl->etw_kw & ev->EventHeader.EventDescriptor.Keyword)) {
				/*
				 * each event might trigger multiple probes.
				 * so set current pid & tid for each probe, since
				 * the current process might change if the session Queue has
				 * filled up, which will trigger sending of a probe.
				 */
				HANDLE *lock = dtrace_etw_set_cur(ev->EventHeader.ProcessId, ev->EventHeader.ThreadId,
...
sdt_add_providers(etw_provinfo_t* lprov)
{
	...
	do {
		...
		if (dtrace_register(prov->sdtp_name, prov->sdtp_attr,
		        DTRACE_PRIV_KERNEL, NULL, &sdt_pops, prov, &prov->sdtp_id) != 0) {
...
sdt_etw_procp_cb(PEVENT_RECORD ev, void *data)
{
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (ev->EventHeader.EventDescriptor.Opcode == sdt->sdp_patchval) {
				if (sdt->sdp_patchval == 1) {
					dtrace_etw_probe(sdt->sdp_id, p, 0,
					    0, 0, 0, FALSE);
				} else if (sdt->sdp_patchval == 2) {
					dtrace_etw_probe(sdt->sdp_id, p->exitval, 0,
					    0, 0, 0, FALSE);
				}
			}
		}
...
sdt_etw_diskio_cb(PEVENT_RECORD ev, void *data)
{
	...
	case 15:	//Flush Initiate
		...
		dtrace_etw_td_find(ev->EventHeader.ProcessId, d2->IssuingThreadId, ETW_SET_CURRENT);
		buf->b_irpaddr = d2->Irp;
		buf->b_flags = opcode == 12 ? 1 : (opcode == 13 ? 2 : 4);
		break;
	case 14:	//flush done
		d3 = (struct DiskIo_TypeGroup3 *) ev->UserData;
		dtrace_etw_td_find(ev->EventHeader.ProcessId, d3->IssuingThreadId, ETW_SET_CURRENT);
	...
	case 11:		//Write Complete
		...
		//XXXX set threadid
		dtrace_etw_td_find(ev->EventHeader.ProcessId, d1->IssuingThreadId, ETW_SET_CURRENT);
...
sdt_etw_tcpip_cb(PEVENT_RECORD ev, void *data)
{
	...
	if (ev->EventHeader.EventDescriptor.Version == 0) {
		...
		case 14:
		case 15: {
			struct TcpIp_V0_TypeGroup1 *msg = (struct TcpIp_V0_TypeGroup1 *) ev->UserData;
			ip->ti_ver = AF_INET;
		...
		switch(opcode) {
		case 10: {
			struct TcpIp_SendIPV4 *msg = (struct TcpIp_SendIPV4 *) ev->UserData;
			ip->ti_ver = AF_INET;
			ip->ti_addr.ip4.daddr = msg->daddr;
			ip->ti_addr.ip4.saddr = msg->saddr;
			ip->ti_dport = msg->dport;
		...
		case 26: {
			struct TcpIp_SendIPV6 *msg = (struct TcpIp_SendIPV6 *) ev->UserData;
			ip->ti_ver = AF_INET6;
		...
		case 16:
		case 18: {
			struct TcpIp_TypeGroup1 *msg = (struct TcpIp_TypeGroup1 *) ev->UserData;
			ip->ti_ver = AF_INET;
			ip->ti_addr.ip4.daddr = msg->daddr;
			ip->ti_addr.ip4.saddr = msg->saddr;
			ip->ti_dport = msg->dport;
		...
		case 12:
		case 15: {
			struct TcpIp_TypeGroup2 *msg = (struct TcpIp_TypeGroup2 *) ev->UserData;
			ip->ti_ver = AF_INET;
			ip->ti_addr.ip4.daddr = msg->daddr;
			ip->ti_addr.ip4.saddr = msg->saddr;
			ip->ti_dport = msg->dport;
		...
		case 32:
		case 34: {
			struct TcpIp_TypeGroup3 *msg = (struct TcpIp_TypeGroup3 *) ev->UserData;
			ip->ti_ver = AF_INET6;
			ip->ti_dport = msg->dport;
			ip->ti_sport = msg->sport;
			ip->ti_addr.ip6.daddr = msg->daddr;
		...
		case 28:
		case 31: {
			struct TcpIp_TypeGroup4 *msg = (struct TcpIp_TypeGroup4 *) ev->UserData;
			ip->ti_ver = AF_INET6;
			ip->ti_dport = msg->dport;
			ip->ti_sport = msg->sport;
			ip->ti_addr.ip6.daddr = msg->daddr;
		...
		case 17: {
			struct TcpIp_Fail *msg = (struct TcpIp_Fail *) ev->UserData;
		...
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_patchval == opcode) {
				if (sdt->sdp_patchval == 17) {
					dtrace_etw_probe(sdt->sdp_id, fail.ti_proto, fail.ti_code,
					    0, 0, 0, FALSE);
				} else {
...
sdt_etw_udpip_cb(PEVENT_RECORD ev, void *data)
{
	...
	if (ev->EventHeader.EventDescriptor.Version == 0) {
		...
		case 10:
		case 11: {
			struct UdpIp_V0_TypeGroup1 *msg = (struct UdpIp_V0_TypeGroup1 *) ev->UserData;
			ip->ui_ver = AF_INET;
		...
		case 10:
		case 11: {
			struct UdpIp_TypeGroup1 *msg = (struct UdpIp_TypeGroup1 *) ev->UserData;
			ip->ui_ver = AF_INET;
			ip->ui_dport = msg->dport;
			ip->ui_sport = msg->sport;
			ip->ui_addr.ip4.daddr = msg->daddr;
		...
		case 26:
		case 27: {
			struct UdpIp_TypeGroup2 *msg = (struct UdpIp_TypeGroup2 *) ev->UserData;
			ip->ui_ver = AF_INET6;
			ip->ui_dport = msg->dport;
			ip->ui_sport = msg->sport;
		...
		case 17: {
			struct UdpIp_Fail *msg = (struct UdpIp_Fail *) ev->UserData;
			fail.ti_code = msg->FailureCode;
			fail.ti_proto = msg->Proto;
			break;
		}
...
sdt_etw_fileio_cb(PEVENT_RECORD ev, void *data)
{
	...
	case 32:
	case 35: {
		struct FileIo_Name *fio = (struct FileIo_Name *) ev->UserData;
		info->f_name = dtrace_etw_get_fname(fio->FileObject);
		break;
	}
	case 64: {
		struct FileIo_Create *fio = (struct FileIo_Create *) ev->UserData;
	...
	case 74:
	case 75: {
		struct FileIo_Info *fio = (struct FileIo_Info *) ev->UserData;
		info->f_fileobj =  fio->FileObject;
		info->f_irpptr = fio->IrpPtr;
		info->f_tid = fio->TTID;
	...
	case 66:
	case 73: {
		struct FileIo_SimpleOp *fio = (struct FileIo_SimpleOp *) ev->UserData;
		info->f_fileobj =  fio->FileObject;
		info->f_irpptr = fio->IrpPtr;
		info->f_tid = fio->TTID;
		info->f_name = dtrace_etw_get_fname(fio->FileKey);
	...
	case 67:	//read
	case 68: {	//write
		struct FileIo_ReadWrite *fio = (struct FileIo_ReadWrite *) ev->UserData;
		info->f_fileobj =  fio->FileObject;
		info->f_irpptr = fio->IrpPtr;
		info->f_tid = fio->TTID;
		info->f_name = dtrace_etw_get_fname(fio->FileKey);
	...
	case 72:
	case 77: {
		struct FileIo_DirEnum *fio = (struct FileIo_DirEnum *) ev->UserData;
		info->f_fileobj =  fio->FileObject;
		info->f_irpptr = fio->IrpPtr;
		info->f_tid = fio->TTID;
		info->f_name = dtrace_etw_get_fname(fio->FileKey);
	...
	case 76: {
		struct FileIo_OpEnd *fio = (struct FileIo_OpEnd *) ev->UserData;
		info->f_irpptr = fio->IrpPtr;
		info->f_extinfo = fio->ExtraInfo;
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_patchval == opcode) {
				if (sdt->sdp_patchval == 72 || sdt->sdp_patchval == 77) {
			...
			} else if ((opcode == 32 && sdt->sdp_patchval == 64) || //XXXX
...
sdt_etw_reg_cb(PEVENT_RECORD ev, void *data)
{
	...
	case 0: {
		struct Registry_V0_TypeGroup1 *reg = (struct Registry_V0_TypeGroup1 *) ev->UserData;
	...
	case 1: {
		struct Registry_V1_TypeGroup1 *reg = (struct Registry_V1_TypeGroup1 *) ev->UserData;
		rinfo->r_status = reg->Status;
		rinfo->r_handle = reg->KeyHandle;
		rinfo->r_time = reg->ElapsedTime;
	...
	case 2: {
		struct Registry_TypeGroup1 *reg = (struct Registry_TypeGroup1 *) ev->UserData;
		rinfo->r_status = reg->Status;
		rinfo->r_handle = reg->KeyHandle;
...
sdt_etw_pf_cb(PEVENT_RECORD ev, void *data)
{
	...
	case 14:
	case 15: {
		struct PageFault_TypeGroup1 *vm = (struct PageFault_TypeGroup1 *) ev->UserData;
		vinfo.va = vm->VirtualAddress;
		vinfo.pc = vm->ProgramCounter;
		break;
	}
	case 32: {
		struct PageFault_HardFault *vm = (struct PageFault_HardFault *) ev->UserData;
	...
	case 98:
	case 99: {
		struct PageFault_VirtualAlloc *vm = (struct PageFault_VirtualAlloc *) ev->UserData;
	...
	case 105: {
		struct PageFault_ImageLoadBacked *vm = (struct PageFault_ImageLoadBacked *) ev->UserData;
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_patchval == opcode) {
				...
				case 14:
				case 15: {
					dtrace_etw_probe(sdt->sdp_id, vinfo.va, vinfo.pc,
					    0, 0, 0, FALSE);
					break;
				}
				case 32: {	//hard fault
					dtrace_etw_probe(sdt->sdp_id, vinfo.fname, vinfo.va,
					    vinfo.time, vinfo.rdoffset, vinfo.tid, FALSE);
					break;
				}
				case 98:	//virtal alloc
				case 99: {	// free
					dtrace_etw_probe(sdt->sdp_id, vinfo.baseaddr, vinfo.pid,
					    vinfo.regsz, vinfo.flags, 0, FALSE);
					break;
				}
				case 105: {		//img load
					dtrace_etw_probe(sdt->sdp_id, vinfo.fname, vinfo.flags,
					    vinfo.devchar, vinfo.filechar, 0, FALSE);
					break;
				}
				default:
					ASSERT(0);
...
sdt_etw_sched_cb(PEVENT_RECORD ev, void *data)
{
	 ...
	 */
	if (opcode == 36) {
		/* etw buffer cpu no represents the cpu of the context switch*/
		/* any thread stack represents the new thread stack, or the stack
		  when it previously went into wait mode.So the off cpu stack will
	...
	sdt_probe_t *tsdt, *sdt =
	    sdt_probetab[SDT_ADDR2NDX(ev->EventHeader.ProviderId.Data1)];
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (opcode == 36 && sdt->sdp_patchval == 35) {
				//off-cpu
				HANDLE *lock = dtrace_etw_set_cur(oldtd->pid, oldtd->tid,
				        ev->EventHeader.TimeStamp.QuadPart-1,
				        ev->BufferContext.ProcessorNumber);
				dtrace_etw_probe(sdt->sdp_id, newtd, newtd->proc,
				    0, 0, 0, TRUE);
				dtrace_etw_reset_cur(lock);
				...
				//wakeup
				HANDLE *lock = dtrace_etw_set_cur(ev->EventHeader.ProcessId,
...
sdt_etw_dpc_cb(PEVENT_RECORD ev, void *data)
{
	...
	if (opcode != 66 && opcode != 68 && opcode != 69)
		return (1);
	struct DPC *dpc = (struct DPC *) ev->UserData;

	sdt_probe_t *sdt = sdt_probetab[SDT_ADDR2NDX(ev->EventHeader.ProviderId.Data1)];
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_patchval == opcode) {
				dtrace_etw_probe(sdt->sdp_id, dpc->InitialTime, dpc->Routine,
				    0, 0, 0, FALSE);
			}
		}
	}
...
sdt_etw_isr_cb(PEVENT_RECORD ev, void *data)
{
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_patchval == opcode) {
				dtrace_etw_probe(sdt->sdp_id, isr->InitialTime, isr->Routine,
...
sdt_etw_syscall_cb(PEVENT_RECORD ev, void *data)
{
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_patchval == opcode) {
				if (opcode == 51)
					dtrace_etw_probe(sdt->sdp_id, *((uint64_t *) ev->UserData), 0,
					    0, 0, 0, FALSE);
				else
					dtrace_etw_probe(sdt->sdp_id, *((uint32_t *) ev->UserData), 0,
					    0, 0, 0, FALSE);
			}
		}
	}
...
sdt_guid_str(GUID *g)
{
	...
	sprintf(str, "{%08x-%04x-%04x-%02x%02x%02x%02x%02x%02x%02x%02x}", g->Data1, g->Data2,
	    g->Data3, g->Data4[0], g->Data4[1],g->Data4[2],
	    g->Data4[3],g->Data4[4],g->Data4[5],g->Data4[6],
	    g->Data4[7]);
...
sdt_etw_diag_cb(PEVENT_RECORD ev, void *data)
{
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_ctl->etw_kw == ev->EventHeader.EventDescriptor.Keyword) {
...
sdt_etw_lost_cb(PEVENT_RECORD ev, void *data)
{
	...
	for (; sdt != NULL; sdt = sdt->sdp_hashnext) {
		if ((uintptr_t)sdt->sdp_patchpoint == ev->EventHeader.ProviderId.Data1) {
			if (sdt->sdp_patchval == opcode) {
				dtrace_etw_probe(sdt->sdp_id, 0, 0, 0, 0, 0, FALSE);
			}
		}
	}
	return (0);
