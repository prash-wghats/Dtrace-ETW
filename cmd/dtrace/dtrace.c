...
dof_prune(const char *fname)
{
	...
	if ((fd = open(fname, O_RDONLY|_O_BINARY)) == -1) {
		/*
...
#else
		} else if (Pexitcode(P) != 0) {
			notice("pid %d exited with status %x\n", pid, Pexitcode(P));
#endif
		} else {
...
main(int argc, char *argv[])
{
	...
	if (g_mode == DMODE_LINK &&
	    (g_oflags & (DTRACE_O_ILP32 | DTRACE_O_LP64)) == 0) {
	    	for (i = 1; i < g_argc; i++) {
	    		int type = gelf_getclass(g_argv[i], 0);
...
#else
			g_ofile = "c:/dtrace/boot/dtrace.dof";
#endif
