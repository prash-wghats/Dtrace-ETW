...
dt_provmod_open(dt_provmod_t **provmod, dt_fdlist_t *dfp)
{
	...
	for (i = 0; i < MAX_PROVIDER; i++) {
		if (dfp->df_ents == dfp->df_size) {
			uint_t size = dfp->df_size ? dfp->df_size * 2 : 16;
			HANDLE *fds = realloc(dfp->df_fds, size * sizeof (HANDLE));
			if (fds == NULL)
				return;
			dfp->df_fds = fds;
			dfp->df_size = size;
		...
		if ((fd = dt_load_provider(providers[i].name, providers[i].attach)) == NULL)
			return;
		if (((prov = malloc(sizeof (dt_provmod_t))) == NULL) ||
		    (prov->dp_name = malloc(strlen(providers[i].name) + 1)) == NULL) {
			free(prov);
			CloseHandle(fd);
			return;
		}

		(void) strcpy(prov->dp_name, providers[i].name);
    ...
    const dtrace_vector_t *vector, void *arg)
{
	...
	if (OpenProcessToken (GetCurrentProcess (), TOKEN_QUERY | TOKEN_ADJUST_PRIVILEGES, &token_hdl)) {
		if (SetPrivilege(token_hdl,SE_DEBUG_NAME, TRUE) == FALSE)
			dt_dprintf("failed to raise privilege\n");
