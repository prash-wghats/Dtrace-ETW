...
#else
	if (FasttrapIoctl(dtp->dt_ftfd, FASTTRAPIOC_MAKEPROBE, ftp) != 0) {
#endif
	    ...
	    fasttrap_probe_spec_t *ftp, const GElf_Sym *symp, uint64_t *stret)
{
	...
	if (Pread(P, text, symp->st_size+4, symp->st_value) != symp->st_size+4) {
...
#if defined(windows)
			if (ftp->ftps_prov_type && off < ftp->ftps_noffs &&
				dt_fpid_return(text, symp->st_size+4, i, size) == 0) 
				ftp->ftps_noffs--;
#endif
		}
	} else {
...
#else
		if (FasttrapIoctl(dis->dtp->dt_ftfd, FASTTRAPIOC_GETINSTR, &instr) != 0)
#endif
...
#if defined(windows)
int
dt_pid_create_cb(struct ps_prochandle *P, dtrace_hdl_t *dtp)
{
	uintptr_t func;

	if (FasttrapIoctl(dtp->dt_ftfd, FASTTRAPIOC_INTRPTCB, &func) != 0) {
		dt_dprintf("fasttrap cb creation ioctl failed: %s\n",
		    strerror(errno));
		return (dt_set_errno(dtp, errno));
	}
