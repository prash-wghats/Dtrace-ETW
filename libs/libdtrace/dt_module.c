...
#else
static void
dt_module_update(dtrace_hdl_t *dtp, const char *fname, const char *bname, uintptr_t base)
{
	dt_module_t *dmp;
	const char *s;
	int l;
	...
	l = strlen(bname);
	if (strncmp(bname+l-4, ".exe", 4) == 0)
		bname = "nt";
	if (bname[0] == 0)
		bname = fname;

	if ((dmp = dt_module_create(dtp, bname)) == NULL) {
		dt_dprintf("failed to open %s: %s\n", fname, strerror(errno));
		return;
	}

	/*
	 * NOTE: In virtalbox, cannot access d drive msg?. Use SYMOPT_FAIL_CRITICAL_ERRORS ??
	 * If SYMOPT_DEFERRED_LOADS is used then the size parameter to SymLoadModuleEx should not be zero.
	 */
	base = SymLoadModuleEx(GetCurrentProcess(), NULL, (char *) fname, NULL, (ULONG_PTR) base, size, NULL, 0);
	if (base == 0) {
	...
	switch (gelf_getclass(fname, dmp->dm_elf)) {
	case ELFCLASS32:
		dmp->dm_ops = &dt_modops_32;
		bits = 32;
		break;
	...
	dt_dprintf("opened %d-bit module %s [%d]\n",
	    bits, dmp->dm_name, dmp->dm_elf);
}
...
dtrace_update(dtrace_hdl_t *dtp)
{
	...
	for (dmp = dt_list_next(&dtp->dt_modlist); dmp != NULL; dmp = dt_list_next(dmp))
		dt_module_unload(dtp, dmp);

	if(EnumDeviceDrivers(drivers, sizeof(drivers), &cbNeeded) && cbNeeded < sizeof(drivers)) {
		char fname[ARRAY_SIZE];
		char bname[ARRAY_SIZE];
		cDrivers = cbNeeded / sizeof(drivers[0]);

		for (i=0; i < cDrivers; i++ ) {
			if(GetDeviceDriverFileName(drivers[i], fname, sizeof(fname) / sizeof(fname[0]))) {
				cleanddpath(fname);
				if (GetDeviceDriverBaseName(drivers[i], bname, sizeof(bname) / sizeof(bname[0])) == 0) {
					bname[0] = 0;
				}
				dt_module_update(dtp, fname, bname, (uintptr_t) drivers[i]);
			}
		}
	}
...
#ifdef windows
	if (etw_lookupkernel_by_addr(dtp->dt_varg, addr, symp, sip) != -1) 
		return (0);
	else {
		...
		if (SymFromAddr(GetCurrentProcess(),  addr, 0, sym) == TRUE) {
			if (sip != NULL && SymGetModuleInfo64(GetCurrentProcess(), addr, &mod)) {
...
#define	ctf_lookup_by_name	pr_ctf_lookup_by_name
int
...
#else
				/*
				 * UDT arguments without typedef are considered.
				 * ex func(struct final_fantasy*)
				 */
				if ((q = strchr(buf, ' ')) != NULL) {
					fp = dt_module_getctflib(dtp, dmp, q+1);
					strcpy(q+1, p+1);
					if (fp == NULL || (id = ctf_lookup_by_name(fp,
				    	    buf)) == CTF_ERR)
						id = CTF_ERR;
