...
dt_pid_per_sym(dt_pid_probe_t *pp, const GElf_Sym *symp, const char *func)
{
	...
	ftp->ftps_pid = pid;
#if defined(windows)
	ftp->ftps_prov_type = pp->dpp_prov_type;
#endif
	...
	if (!pp->dpp_prov_type && !glob && nmatches == 0) {
	...
	} else if (!pp->dpp_prov_type && glob && !isdash) {
    ...
    dt_pcb_t *pcb, dt_proc_t *dpr)
{
	...
	pp.dpp_pcb = pcb;
#if defined(windows)
	if (strstr(pdp->dtpd_provider, "fpid") != NULL) {
		pp.dpp_prov_type = 1;
	} else {
		pp.dpp_prov_type = 0;
	}
...
#else
		return (dt_pid_error(dtp, pcb, dpr, NULL, D_PROC_NAME, "'-' function not supported"));
#endif
	}
...
#endif
#if defined(windows)
		dh.dofhp_addr = pmp->pr_vaddr;
		dh.dofhp_dof = (uintptr_t) sym.st_value;;
		dh.dofhp_pid = Ppid(P);
		dt_pid_objname(dh.dofhp_mod, sizeof (dh.dofhp_mod),
		    0, mname);
		if (DtraceIoctl(0, DTRACEHIOC_ADDDOF, &dh) != 0)
...
dt_pid_create_probes(dtrace_probedesc_t *pdp, dtrace_hdl_t *dtp, dt_pcb_t *pcb)
{
	...
	char provname0[DTRACE_PROVNAMELEN];
#endif
	assert(pcb != NULL);
	...
	if (dtp->dt_ftfd == -1) {
#else
	if (dtp->dt_ftfd == INVALID_HANDLE_VALUE) {
#endif
...
#if defined (windows)
	(void) snprintf(provname0, sizeof (provname0), "fpid%d", (int)pid);
	if (gmatch(provname, pdp->dtpd_provider) != 0 || gmatch(provname0, pdp->dtpd_provider) != 0) {
#else
...
#if defined (windows)
		if (Psetprov(P, !strcmp(provname0, pdp->dtpd_provider)) != 0) {
			return (-1);
		}
#endif
	 ...
	 */
	if (strcmp(provname, pdp->dtpd_provider) != 0 && strcmp(provname0, pdp->dtpd_provider) != 0) {
...
dt_pid_create_probes_module(dtrace_hdl_t *dtp, dt_proc_t *dpr)
{
	...
	(void) snprintf(provname, sizeof (provname), "pid%d",
	    (int)dpr->dpr_pid);
#if defined(windows)
	(void) snprintf(provname0, sizeof (provname0), "fpid%d",
	    (int)dpr->dpr_pid);
#endif
	for (pgp = dt_list_next(&dtp->dt_programs); pgp != NULL;
		...
		for (stp = dt_list_next(&pgp->dp_stmts); stp != NULL;
			...
			if (strcmp(provname, pdp->dtpd_provider) != 0 &&
				strcmp(provname0, pdp->dtpd_provider) != 0 &&
