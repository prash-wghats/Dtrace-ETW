...
dt_preproc(dtrace_hdl_t *dtp, FILE *ifp)
{
	 ...
	 * it still sees the newline, ensuring that #line values are correct.
	 */
	if (isatty(fileno(ifp)) == 0 && (off = _ftelli64(ifp)) != -1) {
		if ((c = fgetc(ifp)) == '#' && (c = fgetc(ifp)) == '!') {
			for (off += 2; c != '\n'; off++) {
				if ((c = fgetc(ifp)) == EOF)
					break;
		...
		(void) fflush(ifp);
		(void) _fseeki64(ifp, off, SEEK_SET);
	}
	...
	Ihand = (HANDLE)_get_osfhandle(fileno(ifp));
	Ohand = (HANDLE)_get_osfhandle(fileno(ofp));
	Ehand = GetStdHandle(STD_ERROR_HANDLE); 
	...
	if (!CreateProcess(NULL, cmdl, NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
         	dt_dprintf("dt_preproc:CreateProcess, failed to create :%s", cmdl);
         	goto err;
        }
        
        WaitForSingleObject(pi.hProcess, INFINITE);
        GetExitCodeProcess(pi.hProcess, &estat);
